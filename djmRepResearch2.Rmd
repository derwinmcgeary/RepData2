...
# Floods and Hurricanes cause more property damage, while Tornados, Heat, and Flooding have the greatest impact on human health
...
Public health impact was assessed by considering the number of injuries and deaths reported with each event. No effort has been made to combine these factors. Tornados, excessive heat, and simply heat are the most lethal events. The greatest number of injuries is caused by tornados too, followed by floods, excessive heat, and thunderstorms.

## Data processing

We will be interested in the columns `EVTYPE`, `CROPDMG`, `CROPDMGEXP`, `PROPDMG`, `PROPDMGEXP`, `INJURIES`, and `FATALITIES`. So we can read in the data and check how complete those columns are.
...
```{r}
hist(as.Date(stormData$BGN_DATE,"%m/%d/%Y"), breaks="years")
```
