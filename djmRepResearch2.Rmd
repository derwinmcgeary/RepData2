...
So, if we view the economic damage by event, ordered.

```{r}
library(ggplot2)
ggplot(stormEconAgg, aes(x=reorder(EVTYPE,TotalDamage), y=TotalDamage)) + labs(title = "Economic Damage due to Storm Events") geom_bar(stat='identity') + coord_flip()
```
We can see that SOMETHIAGNSADGOKAERGH
```{r}
ggplot(strmHealth, aes(x=reorder(EVTYPE,Number), y=Number)) + labs(title = "Public Health Effects of Storm Events") 
+ geom_bar(stat='identity') + facet_grid(Type ~ .) + coord_flip()
```
We can see that ASEGFIJAERLHGKNARGAL.